...
public class MsSQLDAOFactory extends DAOFactory {
	private static String lookupName = "java:jboss/datasources/MaParoisseDS";
	private static MsSQLDAOFactory inst = new MsSQLDAOFactory();
	...
	public static MsSQLDAOFactory getInst() {
		return inst;
	}
...
//	private static String createDSName(String dsName) {
//		String st = System.getProperty("openejb.home");
//		if(st != null){
//			return "java:openejb/Resource/" + dsName;
//		}
//		return "java:" + dsName;
//	}
	...
	public AddressDAO getAddressDAO(AEConnection aeConnection) throws AEException {
		return new AddressDAO(getConnection(aeConnection));
	}

	/* (non-Javadoc)
	 * @see eu.agileeng.persistent.dao.DAOFactory#getDocumentDAO(eu.agileeng.persistent.AEConnection)
	 */
	@Override
	public AEDocumentDAO getDocumentDAO(AEDocumentType documentType, AEConnection aeConnection) throws AEException {
		AEDocumentDAO docDAO = null;
		if(documentType.getSystemID() == AEDocumentType.System.FinDuTravail.getSystemID()) {
			docDAO = new FinDuTravailDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.CertificatDeTravail.getSystemID()) {
			docDAO = new FinDuTravailDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.ContractDeTravail.getSystemID()) {
			docDAO = getCotractDeTravailDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.ContractDeTravailAnex.getSystemID()) {
			docDAO = getCotractDeTravailDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.AccidentDuTravail.getSystemID()) {
			docDAO = getAccidentDuTravailDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.ArretDeTravail.getSystemID()) {
			docDAO = getAccidentDuTravailDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.Rib.getSystemID()) {
			docDAO = getRibDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.AEPurchaseInvoice.getSystemID()) {
			docDAO = getAETradeDocumentDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.AEPurchaseInvoiceFNP.getSystemID()) {
			docDAO = getAETradeDocumentDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.AESaleInvoice.getSystemID()) {
			docDAO = getAETradeDocumentDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.AECheque.getSystemID()) {
			docDAO = getAETradeDocumentDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.AEVirement.getSystemID()) {
			docDAO = getAETradeDocumentDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.AEPrelevement.getSystemID()) {
			docDAO = getAETradeDocumentDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.AELCR.getSystemID()) {
			docDAO = getAETradeDocumentDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.PERIMES.getSystemID()) {
			docDAO = getAETradeDocumentDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.STOCKS.getSystemID()) {
			docDAO = getAETradeDocumentDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.SUIVI_GPL.getSystemID()) {
			docDAO = getAETradeDocumentDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.DONNEES.getSystemID()) {
			docDAO = getAETradeDocumentDAO(getConnection(aeConnection));
		} else if(documentType.getSystemID() == AEDocumentType.System.BordereauParoisse.getSystemID()
				|| documentType.getSystemID() == AEDocumentType.System.Cerfa_11580_03.getSystemID()
				|| documentType.getSystemID() == AEDocumentType.System.ETAT_ANNUEL_DES_COMPTES.getSystemID()
				|| documentType.getSystemID() == AEDocumentType.System.ETAT_ANNUEL_DES_COMPTES_DRAFT.getSystemID()) {
			docDAO = getAEStatementDAO(getConnection(aeConnection));
		} else {
			throw new AEException("Internal Error: AEDocumentDAO cannot be instantiated.\nReason : Unknown AEDocumentType");
		} 
		return docDAO;
	}
